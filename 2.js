...
//Funkcija treba da vrati NOVI NIZ čiji će elementi da budu novi objekti za svaku ocenu 1-10 zasebno. Primer strukture i podataka:
...
function rez(objekat){
	var nizRezultati=[];

	for (var i = 1; i <=10; i++) {
		var count=0;
		for (var j = 0; j < objekat.ocenePolaznika.length; j++) {

			if (i==objekat.ocenePolaznika[j]) {
				count += 1;

			}
		}
		nizRezultati.push({["rate_" + i] : count});

	}
	console.log(nizRezultati);
